...
const Header = () => {
  ...
  const handleLogout = async () => {
    const result = await postData("/auth/student/logout");
    if (result.success) {
      setUser(null);
    }
  };
            ...
            <ul className="menu menu-horizontal flex items-center align-middle">
                        ...
                        {user !== null && <li className='mx-2'><NavLink to="/logout">Logout</NavLink></li>} */}
              {
                <li className="mx-2 dropdown dropdown-end">
                  ...
                  <ul
                    tabIndex={0}
                    className="menu menu-sm bg-green dropdown-content mt-3 z-[1] p-2 shadow bg-white rounded-box w-52"
                  >
                    <li>
                      <Link
                        to="user-profile"
                        className="justify-between text-success"
                      >
                        Profile
                    ...
                    <li>
                      <Link to="enrolled-semesters" className="text-success">
                        Semesters
                      </Link>
                    </li>
                    {/* <li><Link to='courses' className='text-success'>Courses</Link></li> */}
                    <li onClick={handleLogout}>
                      <a className="text-success">Logout</a>
                    </li>
                  </ul>
                </li>
              }
